...
test(`To use the functions for the playground methods`, async function ({ page }, testInfo) {
    ...
    for (let val = 0; val < courses.length; val++) {
        const eachWebElement = await locateElement(page, `//input[contains(@id,'auto-complete_input')]`)
        type(eachWebElement,courses[val])
        await eachWebElement.press(' ')

        const eachValue = await locateElement(page, `//li[@data-item-value='${courses[val]}']`)
...
async function type(locatorValue:Locator,value:string) {
